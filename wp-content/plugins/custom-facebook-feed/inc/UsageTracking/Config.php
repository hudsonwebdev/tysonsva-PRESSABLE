...
class Config {
	 ...
	 */
	const OPTION_TRACKING = 'cff_usage_tracking';
	 ...
	 */
	const OPTION_SITE_TOKEN = 'cff_smash_usage_tracking_site_token';

	/**
	 * Option key: schedule metadata (optional).
	 */
	const OPTION_SCHEDULE = 'cff_smash_usage_tracking_schedule';

	/**
	 * Option key: dates when plugin was active (Y-m-d), for days_active metric.
	 */
	const OPTION_ACTIVE_DATES = 'cff_smash_usage_active_dates';

	/**
	 * Option key: last N session durations in seconds, for session_duration metric.
	 */
	const OPTION_SESSION_DURATIONS = 'cff_smash_usage_session_durations';
	 ...
	 */
	const CRON_HOOK = 'cff_free_smash_usage_tracking_cron';
	...
	public static function get_api_url() {
		$url = defined( 'CFF_SMASH_USAGE_TRACKING_API_URL' ) ? CFF_SMASH_USAGE_TRACKING_API_URL : '';
		return (string) apply_filters( 'cff_smash_usage_tracking_api_url', $url );
	}
