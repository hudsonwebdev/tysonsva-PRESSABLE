 ...
 *
 * @package CustomFacebookFeed\UsageTracking
 * @since 4.0
 */

namespace CustomFacebookFeed\UsageTracking;

use CustomFacebookFeed\UsageTracking\Core\RegisterSite;
use CustomFacebookFeed\UsageTracking\Core\Sender;
use CustomFacebookFeed\UsageTracking\Core\PayloadBuilder;
use CustomFacebookFeed\UsageTracking\Core\Scheduler;
use CustomFacebookFeed\UsageTracking\Facebook\FacebookFreeReporter;
...
class SmashUsageTracking {
	...
	public function __construct() {
		$this->reporter        = new FacebookFreeReporter();
		...
		add_action( 'wp_ajax_cff_smash_usage_record_event', array( $this, 'ajax_record_event' ) );
		add_action( 'wp_ajax_cff_smash_usage_record_session', array( $this, 'ajax_record_session' ) );
	...
	/**
	 * Record active day and optionally settings_page_viewed when on a CFF admin page.
	 */
	public function maybe_record_active_day( $screen ) {
		if ( ! $screen || strpos( $screen->id, 'cff' ) === false ) {
			return;
		}
		if ( ! Config::is_enabled() ) {
			return;
		...
		if ( isset( $screen->id ) && strpos( $screen->id, 'cff-settings' ) !== false ) {
	...
	/**
	 * Enqueue session-duration and record-event script on CFF admin pages.
	 */
	public function enqueue_session_script() {
		...
		$min         = defined( 'CFF_MIN' ) && CFF_MIN ? '.min' : '';
		$script_path = 'admin/assets/js/smash-usage-session' . $min . '.js';
		...
		wp_enqueue_script(
			'cff-smash-usage-session',
			CFF_PLUGIN_URL . $script_path,
			array( 'jquery' ),
			defined( 'CFFVER' ) ? CFFVER : '1.0',
			true
		);
		wp_localize_script(
            'cff-smash-usage-session',
            'cffSmashUsageSession',
            array(
				'ajax_url'    => admin_url( 'admin-ajax.php' ),
				'nonce'       => wp_create_nonce( 'cff_smash_usage_record_session' ),
				'event_nonce' => wp_create_nonce( 'cff_smash_usage_record_event' ),
            )
        );
	}
