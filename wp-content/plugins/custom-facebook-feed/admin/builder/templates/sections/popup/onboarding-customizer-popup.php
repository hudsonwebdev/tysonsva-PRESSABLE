...
<div v-for="tooltip in customizeScreensText.onboarding.tooltips"
	...
	<button type="button" aria-label="<?php esc_attr_e( 'Close', 'custom-facebook-feed' ); ?>" class="cff-fb-popup-cls" @click.prevent.default="onboardingClose()">
	...
	<div class="sb-onboarding-bottom-row">
		...
		<div class="sb-previous-next-wrap">
			<div class="cff-fb-wrapper">
				<button type="button" class="cff-onboarding-previous cff-fb-hd-btn cff-btn-grey sb-button-small sb-button-left-icon" v-bind:data-active="tooltip.step === 1 ? 'false' : 'true'" @click.prevent.default="onboardingPrev">
				...
				<button type="button" v-if="customizeScreensText.onboarding.tooltips.length > tooltip.step" class="cff-onboarding-next cff-fb-hd-btn cff-btn-grey sb-button-small sb-button-right-icon" @click.prevent.default="onboardingNext">{{genericText.next}}
					<svg aria-hidden="true" focusable="false" width="6" height="8" viewBox="0 0 6 8" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M1.66656 0L0.726562 0.94L3.7799 4L0.726562 7.06L1.66656 8L5.66656 4L1.66656 0Z" fill="#141B38"/>
					</svg>
				</button>
				<button type="button" v-if="customizeScreensText.onboarding.tooltips.length === tooltip.step" class="cff-onboarding-finish sb-button-small cff-btn-grey cff-fb-hd-btn" @click.prevent.default="onboardingClose">{{genericText.finish}}</button>
			</div>
		</div>
	</div>
