...
function wpfc_em_calendar_search($args){
	if( defined('EM_VERSION') && $args['type'] == 'event' ){
		...
		<select name="country" class="em-events-search-country wpfc-taxonomy">
			...
			?>
			 <option value="<?php echo esc_attr($em_country[0]); ?>" <?php echo (!empty($country) && $country == $em_country[0]) ? 'selected="selected"':''; ?>><?php echo esc_html($countries[$em_country[0]]); ?></option>
			<?php endforeach; ?>
		</select>
		<!-- END Country Search -->
		<?php endif; ?>
