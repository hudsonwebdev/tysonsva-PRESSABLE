<div class="sbi-fb-sourcelist-pp-ctn sb-fs-boss sbi-fb-center-boss"
     role="dialog" aria-modal="true" aria-label="<?php esc_attr_e( 'Connected sources', 'instagram-feed' ); ?>"
    ...
    <div class="sbi-fb-source-popup sbi-fb-popup-inside sbi-fb-source-pp-customizer">
        <button type="button" class="sbi-fb-popup-cls" aria-label="<?php esc_attr_e( 'Close', 'instagram-feed' ); ?>" @click.prevent.default="toggleFeedTypesSourcesPopup()">
        ...
        <div class="sbi-fb-sourcelist-pp">
            <div class="sbi-fb-srcslist-ctn sbi-fb-fs" role="group" aria-label="<?php esc_attr_e('Sources', 'instagram-feed'); ?>" v-if="sourcesList.length > 0">
                ...
                <div class="sbi-fb-srcs-item sbi-fb-onbrd-tltp-parent sbi-fb-onbrd-tltp-center-top sbi-fb-onbrd-tltp-hover"
                     role="button"
                     :tabindex="checkSourceDisabledPopup(source) ? -1 : 0"
                     :aria-label="source.username"
                     :aria-disabled="checkSourceDisabledPopup(source) ? 'true' : 'false'"
                     v-for="(source, sourceIndex) in sourcesList" @click.prevent.default="selectSourcePopup(source)"
                     @keydown.enter.prevent.default="selectSourcePopup(source)"
                     @keydown.space.prevent.default="selectSourcePopup(source)"
