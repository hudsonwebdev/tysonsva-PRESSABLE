...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
            ...
            // Use admin_init for the 'silent' option processing code.
			add_action( 'admin_init', array( $this, 'init_silent' ) );
		}
		...
		public function init() {
            ...
            // Don't build the menus and notices if operating silently.
			if ( true !== $this->is_tgmpa_complete() && true !== $this->silent ) {
				// Sort the plugins.
        ...
        public function init_silent(){
	        // Proceed only if we have plugins to handle.
	        if ( empty( $this->plugins ) || ! is_array( $this->plugins ) ) {
		        return;
	        }

	        // Handle silent install/update of plugins.
	        if( true !== $this->is_tgmpa_complete() && true === $this->silent){

		        foreach($this->plugins as $item) {

			        //do_plugin_install
			        $_GET['plugin'] = $item['slug'];


			        // Display the 'Install' action link if the plugin is not yet available.
			        if ( ! $this->is_plugin_installed( $item['slug'] ) ) {

				        $_GET['tgmpa-install'] = 'install-plugin';
				        $action_handler = 'tgmpa-install';
				        $nonce = wp_create_nonce($action_handler);
				        ...
				        // Update plugin.
				        if ( false !== $this->does_plugin_have_update( $item['slug'] ) && $this->can_plugin_update( $item['slug'] ) ) {

					        $_GET['tgmpa-update'] = 'update-plugin';
					        $action_handler = 'tgmpa-update';
					        $nonce = wp_create_nonce($action_handler);
					        $_GET['tgmpa-nonce'] = $nonce;
				        ...
				        // Activate plugin.
				        if ( $this->can_plugin_activate( $item['slug'] ) ) {

					        $this->populate_file_path($item['slug']);

					        $this->activate_single_plugin($this->plugins[$item['slug']]['file_path'], $item['slug'], true);
		...
		protected function do_plugin_install() {
			...
			if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
                ...
                $nonce = $_GET['tgmpa-nonce'] ?? false;

				if(!wp_verify_nonce( $nonce, 'tgmpa-'.$install_type )) {
					return false;
				}
